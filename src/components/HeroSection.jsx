    ...
    const type = () => {
      ...
      if (!isDeleting) {
        ...
        if (displayText.length === currentRole.length) {
          timeout = setTimeout(() => {
            setIsDeleting(true);
          }, 2000); // Exactly 2 seconds pause when text is fully typed
          return;
        }
        
        // Very fast typing speed
        const typingSpeed = 2 + Math.random() * 10; 
        ...
        if (displayText.length === 0) {
          ...
          timeout = setTimeout(type, 100); // Quick transition to next phrase
          return;
        }
        
        // Very fast deletion speed
        timeout = setTimeout(type, 5);
      }
    };
